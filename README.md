...
刚开始的时候是自己认真听认真打分，很喜欢的会打5分，一般的也会打2分。<br>
但是后来发现自己打分很难每周获得高分数，导致一个月几乎每天打分才能刚好够到320分的及格线。<br>
其次很多时候每周小结出来一首歌4分评的人最多，但是最后平均分是3.4分导致这首歌我没有额外的分拿。<br>
...
网页里的「前往评定」「评定今日歌曲」等按钮在电脑浏览器里**是点不动的**
（它们调用的是 App 内部路由 `orpheus://`），所以这个Skill使用URL（如有更好的办法欢迎沟通）
